...
public class SQL {
    ...
    public static void deletePlayerData(String string) {
        ProxyServer.getInstance().getScheduler().runAsync(DBABungeePlugin.plugin, () -> PoolManager.execute(connection -> {
        ...
        ProxyServer.getInstance().getScheduler().runAsync(DBABungeePlugin.plugin, () -> {
            ...
            PoolManager.execute(connection -> {
                ...
                if (resultSet.next()) {
                    ...
                    if (DBABungeePlugin.plugin.getPlayerDataList().searchPlayer(proxiedPlayer.getName()) == null) {
                        ...
                        DBABungeePlugin.plugin.getPlayerDataList().addPlayer((PlayerData) object);
                        callbackSQL.done(true);
                        resultSet.close();
                        preparedStatement.close();
                        connection.close();
                        ...
                        object = new PlayerData(resultSet.getString("uuid"), resultSet.getString("name"), resultSet.getString("email"), resultSet.getString("reg_ip"), resultSet.getString("log_ip"), resultSet.getString("password"), resultSet.getString("salt"), resultSet.getDate("firstjoin"), resultSet.getDate("lastjoin"), resultSet.getBoolean("premium"), resultSet.getBoolean("valid"), resultSet.getString("server"), resultSet.getBoolean("lwlogged"), true);
                        DBABungeePlugin.plugin.getPlayerDataList().modifyPlayer((PlayerData) object);
                        callbackSQL.done(true);
                        resultSet.close();
                        preparedStatement.close();
                        connection.close();
                    }
                } else if (DBABungeePlugin.plugin.getPlayerDataList().searchPlayer(proxiedPlayer.getName()) == null) {
    ...
    public static void registerIP(ProxiedPlayer proxiedPlayer, int n) {
        ProxyServer.getInstance().getScheduler().runAsync(DBABungeePlugin.plugin, () -> PoolManager.execute(connection -> {
            ...
            if (!resultSet.next()) {
                statement.executeUpdate("INSERT INTO playerip (`ip`, `accounts`, `playing`, `max_accounts`, `max_playing`) VALUES ('" + string + "', '1', '" + n + "', '" + DBABungeePlugin.plugin.getConfigLoader().getIntegerCFG("Options.IPChecker.MaxAccountsDefault") + "', '" + DBABungeePlugin.plugin.getConfigLoader().getIntegerCFG("Options.IPChecker.MaxPlayingDefault") + "');");
                resultSet.close();
                statement.close();
                connection.close();
                return null;
